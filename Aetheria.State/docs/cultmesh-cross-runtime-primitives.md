...
- `E:/Projects/CultMath/src/CultMath/float2.cs` and `float3.cs` define the shared HLSL-shaped vector primitives. `E:/Projects/CultLib/src/GameCult.Geometry/CultGeometryPrimitives.cs` builds `CultRect`, `CultCircle`, and `CultSphere` spatial values from those vectors.
...
- `Packages/org.gamecult.aetheria.state/Runtime/AetheriaRuntimePlayerSettingsSurfaceBuilder.cs` builds Aetheria runtime surface source data, then lowers it to the canonical `GameCult.Eve.Surface.EveSurfaceDocument` published through CultMesh. CultMesh owns state and operation binding descriptors; Eve owns the surface document and CultUI composition model. Mesh no longer defines a parallel Eve surface DTO.
...
- `Packages/org.gamecult.aetheria.state/Runtime/AetheriaRuntimeDaemonRenderQueries.cs` exposes daemon render gravity/body query overloads around `GameCult.Geometry.CultRect` in Aetheria XY space. `AetheriaRuntimeXzRect` remains the Unity-facing compatibility adapter.
...
CultMath and GameCult.Geometry provide deterministic cross-runtime primitives for vectors, rects, circles, spheres, transforms, scalar tolerances, and spatial query inputs. In C#, `CultMath.float2` and `CultMath.float3` own vector semantics while `GameCult.Geometry.CultRect`, `CultCircle`, and `CultSphere` own spatial geometry; in TS this starts as `CultMeshVec2`, `CultMeshRect`, and `CultMeshViewportRequest` in `cultmesh-ts`. SoA does not require splitting a vector into unrelated conceptual values; the layout can still be cache-efficient while the API treats position, velocity, and acceleration as meaningful vector values.
