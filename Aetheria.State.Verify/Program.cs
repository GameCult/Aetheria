...
static void RequireEveRuntimeBootstrap(string root)
{
    ...
    var requiredPresenterSymbols = new[]
    {
        ...
        "CultMeshReactiveDocument<EveSurfaceDocument>",
        ...
        "UseShell = false",
        "MountSurface(AetheriaRuntimeStateBootReport stateBoot, EveSurfaceDocument surface)",
        "AetheriaEveUnitySurfaceHost.Render(",
        "control.TrySubmitSurfaceCommand(request, out var daemonEnvelope)",
        ".SurfaceCommandAsync("
    };
    ...
    {
        throw new InvalidOperationException(
            "Aetheria Eve presenter must use provider-native typed state/control/UI access instead of raw client plumbing.");
    }
...
static void RequireInventoryEquippedItemDetailsUseEveSurface(string root)
{
    ...
    var requiredActionBarInventorySymbols = new[]
    {
        ...
        "AetheriaUnityRuntimeClientProvider.Control(\"unity-inventory-menu\")"
    };
    ...
    {
        throw new InvalidOperationException(
            "InventoryMenu no longer submits equipped-item action-bar bindings through native Aetheria control access: " +
...
static void RequireTradeFilterAndRowActionsUseEveSurface(string root)
{
    ...
    var requiredSymbols = new[]
    {
        ...
        "BuildTradeFilterSurfaceModel(",
        "AetheriaRuntimeTradeInteractionSurfaceBuilder.BuildFilters(",
        ...
        "AetheriaRuntimeTradeInteractionSurfaceBuilder.BuildRowActions(",
...
static void RequireMenuDockingUsesManagedTypedSnapshot(string root)
{
    ...
    if (tradeMenu.Contains("CultMeshReactiveDocument<AetheriaRuntimeCurrentDockingDocument> _currentDocking", StringComparison.Ordinal) ||
        ...
        tradeMenu.Contains("CultMeshReactiveDocument<AetheriaRuntimeStationRefitDocument> _stationRefit", StringComparison.Ordinal) ||
        tradeMenu.Contains(".ReactiveStationRefit(\"unity-trade\")", StringComparison.Ordinal) ||
        tradeMenu.Contains("return _stationRefit?.Current", StringComparison.Ordinal) ||
...
static void RequireUnitySharedDocumentAccessorErgonomics(string root)
{
    ...
    var requiredClientSymbols = new[]
    {
        ...
        "public CultMeshDocumentHandle<AetheriaRuntimePlayerSettingsDocument> PlayerSettings",
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must expose named typed document handles for shared and indexed Unity documents, with reactive sync owned by CultMesh handles: " +
            string.Join(", ", missingClientSymbols));
    }
...
static void RequireUnityViewportAndMapReadsUseManagedAccessors(string root)
{
        ...
        StringComparison.Ordinal))
    {
        missingRenderSplatViewportSourceSymbols = missingRenderSplatViewportSourceSymbols
            .Concat(new[] { ".RenderSplatViewportDocuments(viewport,\"unity-render-splat-viewport\")" })
            .ToArray();
    }
        ...
        StringComparison.Ordinal))
    {
        missingZoneRendererViewportSymbols = missingZoneRendererViewportSymbols
            .Concat(new[] { ".ZonePresentationDocuments(viewportBounds,\"unity-zone-renderer\")" })
            .ToArray();
    }
    ...
    if (zoneRenderer.Contains("CultMeshReactiveDocument<AetheriaRuntimeObjectsViewportDocument> _objectsViewport", StringComparison.Ordinal) ||
        zoneRenderer.Contains(".ReactiveObjectsViewport(", StringComparison.Ordinal) ||
        zoneRenderer.Contains("AetheriaRuntimeObjectsViewportSession _objectsViewport", StringComparison.Ordinal) ||
        zoneRenderer.Contains(".ObserveObjects(viewportBounds)", StringComparison.Ordinal))
    {
        throw new InvalidOperationException(
            "ZoneRenderer still routes fallback objects viewports through raw/session wrapper state instead of a managed viewport document.");
    }

    Console.WriteLine("Viewport and map document accessors: Unity reads fixed map/contact state and parameterized viewport state through named managed typed documents");
}
...
static void RequireAetheriaManagedStateAccessorsCoverDomainDocuments(string root)
{
    ...
    if (clientState.Contains("public Task<TDocument> LatestAsync<TDocument>", StringComparison.Ordinal) ||
        ...
        clientState.Contains("CultMeshDocumentCatalog", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeDaemonProviderAdvertisementDocument> ProviderAdvertisementDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeDaemonHealthDocument> HealthDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeDaemonCommandBoundaryDocument> CommandBoundaryDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeVerseAuthorityPolicyDocument> AuthorityPolicyDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeDaemonFrameDocument> DaemonFrameDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeDaemonSoaViewDocument> DaemonSoaViewDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeCatalogSnapshot> CatalogSnapshot(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimePlayerSettingsDocument> PlayerSettingsDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeCurrentEntityDocument> CurrentEntityDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeCurrentDockingDocument> CurrentDocking(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeStationRefitDocument> StationRefitDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public CultMeshDocumentHandle<AetheriaRuntimeStarbridgeSessionSummaryDocument> StarbridgeSummaryDocument(", StringComparison.Ordinal) ||
        clientState.Contains("public Task<CultMeshReactiveDocument<TDocument>> ReactiveAsync<TDocument>", StringComparison.Ordinal) ||
        clientState.Contains("public Observable<TDocument> Watch<TDocument>", StringComparison.Ordinal))
    {
        throw new InvalidOperationException(
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must not reintroduce latest/async fixed-document wrappers; use named reactive typed documents for ordinary gameplay state: " +
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState still exposes nested wrapper state; use flat named typed handles for ordinary gameplay state: " +
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must avoid latest/async viewport wrappers; callers should use named reactive typed viewport documents: " +
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must avoid latest/async scoped wrappers; callers should use named reactive typed scoped documents: " +
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must avoid latest/async player-seat wrappers; callers should use named reactive typed player-seat documents: " +
    ...
    {
        throw new InvalidOperationException(
            "Starbridge player-seat examples must use named parameterized managed document access instead of raw handle walks.");
    }
...
static void RequireDaemonVersePublication(string root)
{
    ...
    if (!runtimeSurfaceDocuments.Contains("public static EveUiSurfaceDocument ToPortableSurface(AetheriaRuntimeSurfaceDocument document)", StringComparison.Ordinal) ||
        !runtimeSurfaceDocuments.Contains("new EveUiCommandTemplate(command.Operation)", StringComparison.Ordinal) ||
        !runtimeSurfaceDocuments.Contains("new EveUiSurfaceComponent(", StringComparison.Ordinal) ||
        runtimeSurfaceDocuments.Contains("GameCult.Mesh.EveSurface", StringComparison.Ordinal))
    {
        throw new InvalidOperationException("Runtime surface documents no longer lower daemon surfaces into canonical Eve surface documents published through CultMesh.");
    }
...
static void RequireCatalogSurfaceUsesManagedRuntimeCatalog(string root)
{
    ...
    if (bridge.Contains("ReadCatalog", StringComparison.Ordinal))
    {
        throw new InvalidOperationException(
...
static void RequireAetheriaRuntimeVerseClientContract(string root)
{
    ...
    {
        throw new InvalidOperationException(
            "AetheriaRuntimeEveSurfaceState.cs is legacy surface-state cruft; daemon UI surfaces must publish GameCult.Eve.Surface.EveSurfaceDocument.");
    }
    ...
    var requiredClientSymbols = new[]
    {
        ...
        "BootstrapLoadoutTemplates",
        "BootstrapPlayerSettings",
        "BootstrapVerseHostSettings",
    ...
    if (!starbridgeSummaryBlock.Contains("RequireManagedCatalog()", StringComparison.Ordinal) ||
        ...
        starbridgeSummaryBlock.Contains("BootstrapRuntimeCatalog", StringComparison.Ordinal) ||
    ...
    var forbiddenClientCompatibilityStoreBypasses = new[]
    {
        ...
        "public AetheriaRuntimeCatalogSnapshot ReadRuntimeCatalog(",
        "public AetheriaRuntimeLoadoutTemplatesDocument ReadLoadoutTemplates(",
        "public AetheriaRuntimePlayerSettingsDocument ReadPlayerSettings(",
        "public AetheriaRuntimeVerseHostSettingsDocument ReadVerseHostSettings(",
        "public Task<AetheriaRuntimeCatalogSnapshot> ReadRuntimeCatalogAsync(",
        "public Task<AetheriaRuntimeLoadoutTemplatesDocument> ReadLoadoutTemplatesAsync(",
        "public Task<AetheriaRuntimePlayerSettingsDocument> ReadPlayerSettingsAsync(",
        "public Task<AetheriaRuntimeVerseHostSettingsDocument> ReadVerseHostSettingsAsync(",
...
static void RequireRuntimeStateReaderOwnsUnityStateAcquisition(string root)
{
    ...
    var unityDaemonRenderDocumentsPath = Path.Combine(root, "Assets", "Scripts", "Gameplay", "AetheriaUnityDaemonRenderDocuments.cs");
    ...
    if (!observedFrameApplier.Contains("private AetheriaRuntimeZoneRenderDocument _lastZoneRender;", StringComparison.Ordinal) ||
        ...
        !observedDaemonState.Contains("new AetheriaRuntimeDaemonRenderView(currentFrame, currentSoaView, currentZoneRender)", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("public sealed class AetheriaUnityDaemonRenderDocuments : IDisposable", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("private readonly CultMeshReactiveDocument<AetheriaRuntimeDaemonFrameDocument> _frame;", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("private readonly CultMeshReactiveDocument<AetheriaRuntimeDaemonSoaViewDocument> _soaView;", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("private readonly CultMeshReactiveDocument<AetheriaRuntimeZoneRenderDocument> _zoneRender;", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("public AetheriaRuntimeDaemonRenderView Current", StringComparison.Ordinal) ||
        !unityDaemonRenderDocuments.Contains("AetheriaRuntimeDaemonRenderView.TryCreateCurrent(", StringComparison.Ordinal) ||
        !daemonObserver.Contains("private AetheriaUnityDaemonRenderDocuments _renderDocuments;", StringComparison.Ordinal) ||
        !daemonObserver.Contains("_renderDocuments?.Current", StringComparison.Ordinal) ||
        !daemonObserver.Contains("AetheriaUnityRuntimeClientProvider.DaemonRenderDocuments(stateBoot, clientId)", StringComparison.Ordinal))
    {
        throw new InvalidOperationException(
    ...
    if (aetheriaClient.Contains("TryReactiveDaemonSoaView()", StringComparison.Ordinal) ||
    ...
    {
        "public CultMeshDocumentHandle<AetheriaRuntimeDaemonFrameDocument> DaemonFrame",
        "public CultMeshDocumentHandle<AetheriaRuntimeCatalogSnapshot> Catalog",
        "public CultMeshDocumentHandle<AetheriaRuntimeCurrentEntityDocument> CurrentEntity",
        "public CultMeshDocumentHandle<AetheriaRuntimeObjectsViewportDocument> ObjectsViewport(",
        "public CultMeshDocumentHandle<AetheriaRuntimeInventoryDocument> Inventory(",
        "public CultMeshDocumentHandle<AetheriaRuntimeStarbridgePlayerSeatDocument> StarbridgePlayerSeat("
    };
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState must expose direct named typed document handles instead of AetheriaClient forwarding shortcuts or per-document session wrappers: " +
    ...
    if (aetheriaClientState.Contains("public Task<TDocument> LatestAsync<TDocument>", StringComparison.Ordinal) ||
        ...
        aetheriaClientState.Contains("public CultMeshDocumentHandle<TDocument> Document<TDocument>", StringComparison.Ordinal) ||
        aetheriaClientState.Contains("public CultMeshReactiveDocument<TDocument> Reactive<TDocument>", StringComparison.Ordinal) ||
        aetheriaClientState.Contains("TryGetDocument<TDocument>", StringComparison.Ordinal) ||
        aetheriaClientState.Contains("CultMeshDocumentCatalog", StringComparison.Ordinal) ||
    ...
    {
        throw new InvalidOperationException(
            "AetheriaClientState still exposes generic document catalogs, latest/watch, or schema-string shortcuts instead of direct named typed CultMesh document handles.");
    }
    ...
    var forbiddenDaemonClientBypassSymbols = new[]
    {
        ...
        "public async Task<AetheriaRuntimeCurrentDocking> CurrentDockingAsync()",
    ...
    if (!daemonObserver.Contains("AetheriaRuntimeStateBoot.Inspect(AetheriaUnityRuntimePaths.GameDataDirectory)", StringComparison.Ordinal) ||
    ...
    {
        throw new InvalidOperationException(
            "AetheriaDaemonObserver must hold a native managed daemon-render document handle and read its current state.");
    }
    ...
    if (daemonObserver.Contains("AetheriaRuntimeStateReader.TryReadDaemonRenderView", StringComparison.Ordinal) ||
        ...
        daemonObserver.Contains("AetheriaClientState ResolveState()", StringComparison.Ordinal) ||
        ...
        daemonObserver.Contains("_daemonSoaView ??= state.ReactiveDaemonSoaView();", StringComparison.Ordinal) ||
    ...
    if (eveSurfacePresenter.Contains("AetheriaUnityRuntimeClientProvider.ResolveClient(", StringComparison.Ordinal) ||
        eveSurfacePresenter.Contains("client.State.CreateEveSurfaceCultMeshStateRefResolver()", StringComparison.Ordinal) ||
    ...
    {
        throw new InvalidOperationException(
            "Aetheria Eve surface presenter still peels through the raw client instead of using provider-native state/ref handles.");
    }
...
static void RequireInventoryLoadoutSaveRequestAuthority(string root)
{
    ...
    if (snapshotDocuments.Contains("ProjectLoadoutTemplateAsync(", StringComparison.Ordinal) ||
        ...
        snapshotDocuments.Contains(".Reactive<AetheriaRuntimeDaemonFrameDocument>()", StringComparison.Ordinal) ||
    ...
    {
        throw new InvalidOperationException(
            "Loadout template save payloads must not sample managed daemon-frame state inside snapshot composition.");
    }
