...
static void RequireAetheriaRuntimeVerseClientContract(string root)
{
    ...
    var requiredClientSymbols = new[]
    {
        ...
        "Aetheria().Daemon.AuthorityPolicy.LatestAsync()",
        "Aetheria().Daemon.ProviderAdvertisement.LatestAsync()",
        "Aetheria().Daemon.Health.LatestAsync()",
        "Aetheria().Daemon.CommandBoundary.LatestAsync()",
        "Aetheria().Daemon.LatestFrame.LatestAsync()",
        "Aetheria().Daemon.LatestSoaView.LatestAsync()",
...
static void RequireRuntimeStateReaderOwnsUnityStateAcquisition(string root)
{
    ...
    {
        throw new InvalidOperationException(
            "AetheriaDaemonObserver no longer routes observed daemon state through the shared Aetheria client facade.");
    }
