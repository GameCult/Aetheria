        ...
        columns.Add(("Owned", 1,
            x => () =>
            {
                if (x.data is HullData)
                    return GameManager.DockedEntity.Children.Count(s => s.Hull.Data.Key.Equals(GameManager.ItemManager.ItemData.RefOf(x.data).Key) && s is Ship {IsPlayerShip: true}).ToString();
